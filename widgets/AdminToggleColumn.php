 ...
 * Time: 6:15 PM
 */
class AdminToggleColumn extends TbDataColumn
{
	/**
	 ...
	 * You may set this property to be false to render a text link instead.
	 */
	public $uncheckedIcon = 'circle-blank';
	...
	protected function initButton()
	{
		if ($this->checkedButtonLabel === null)
			$this->checkedButtonLabel = Yii::t('AdminModule.t9n', 'Uncheck');
		if ($this->uncheckedButtonLabel === null)
			$this->uncheckedButtonLabel = Yii::t('AdminModule.t9n', 'Check');
		if ($this->emptyButtonLabel === null)
			$this->emptyButtonLabel = Yii::t('AdminModule.t9n', 'Not set');

        $me = $this;
		$this->button = array(
            'url' => function($data) use ($me) {
                if(is_callable($me->toggleParams))
                    $params = call_user_func($me->toggleParams,$data,$me);
                else
                    $params = array('id'=>$data->primaryKey,'attribute'=>$me->name);
                return Yii::app()->controller->createUrl($me->toggleAction, $params);
            },
...
function() {
	...
	protected function renderDataCellContent($row, $data)
	{
		...
		if (!$this->displayText)
		{
			...
			echo CHtml::link('<i class="icon-'.$button['icon'].'"></i>', $button['url'], $button['htmlOptions']);
		}
		else
		{
